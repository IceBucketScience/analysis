...
def compute_exposure_pcts(g):
    ...
    for participant in participants_w_min_friends:
        coord_pair = {'exposure_pct': compute_exposure_pct(participant), 'participated': 1 if participant.completed_challenge() else 0}
...
logit = sm.Logit(exposure_pcts['participated'], exposure_pcts['exposure_pct'])
result = logit.fit()
print result.summary()

exposure_pcts.plot(x='exposure_pct', y='participated', kind='scatter')
plt.show()